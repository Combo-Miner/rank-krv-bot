...
module.exports = function (client) {
  ...
  client.on("message", async (message) => {
    ...
    let authorized = [`${config.bot.owner}`, `${config.bot.owner1}`,
 `${config.bot.owner2}`, 
`${config.bot.owner3}`, 
`${config.bot.owner4}`, 
`${config.bot.owner5}`, 
...
if(!authorized.includes(message.author.id)) return;
    ...
    if (client.points.get(key, `level`) < curLevel) {
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .setTimestamp()
        .setDescription(`GG, tu as level up au niveau: **\`${curLevel}\`**! (Points: \`${Math.floor(client.points.get(key, `points`) * 100) / 100}\`) `)
        .setColor("GREEN");
    ...
    if (message.content.toLowerCase().startsWith(`${config.PREFIX}rank`)) {
      ...
      let tempmsg = await message.channel.send(new Discord.MessageEmbed().setColor("RED").setAuthor("Calcule en cours...", "https://cdn.discordapp.com/emojis/769935094285860894.gif"))
      ...
      rank.build()
        .then(async data => {
          ...
          //define embed
          const embed = new Discord.MessageEmbed()
            .setTitle(`Rank de :  ${rankuser.username}`)
            .setColor(color)
